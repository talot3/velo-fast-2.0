const CACHE_NAME = 'velo-pdv-cache-v1';
const ASSETS = [
  'index.html',
  'app.js',
  'styles.css',
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      console.log('[Service Worker PDV] Pré-cacheando recursos essenciais');
    ...
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cache) => {
          if (cache !== CACHE_NAME) {
            console.log('[Service Worker PDV] Removendo cache antigo:', cache);
    ...
    fetch(event.request)
      ...
      .catch(() => {
        ...
        return caches.match(event.request).then((cachedResponse) => {
          ...
          // Se não estiver no cache (ex: fontes externas), falhar graciosamente
